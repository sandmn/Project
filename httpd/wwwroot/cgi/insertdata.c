...
void insert_data(char* name, char* sex, char* tel)
{
    ...
    if(mysql_real_connect(mysql_fd, "127.0.0.1", "root", "yan21188*", "31_class", 3306, NULL, 0) == NULL)//连接mysql
    {
    ...
    //可以对连接上的数据库进行操作了
    char mysql[1024];
    sprintf(mysql, "insert into student_info(name, sex, phone) values(\"%s\", \"%s\", \"%s\")", name, sex, tel);
    printf("sql: %s\n", mysql);
